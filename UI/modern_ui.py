...
class Ui_ModernWindow(object):
    def setupUi(self, MainWindow):
        ...
        # 💥 物理拉伸 1：侧边栏变宽一点，间距从 6 放大到 20，彻底消除拘束感
        ...
        self.nav_buttons = []
        btn_names = ["系统首页", "图片检测", "视频检测", "实时监控", "历史记录", "统计中心", "个人中心", "系统设置", "AI 助手"]
        ...
        for i, name in enumerate(btn_names):
            ...
            # 🚨 实时监控 (💥 核心矩阵升级区 💥)
                ...
                # 顶部置信度控制条
                top_control = QtWidgets.QHBoxLayout()
                self.label_conf_rt = QtWidgets.QLabel("全局报警置信度: 0.60")
                ...
                self.slider_conf_rt.setFixedWidth(200)
                top_control.addWidget(self.label_conf_rt, alignment=QtCore.Qt.AlignVCenter)
                top_control.addWidget(self.slider_conf_rt, alignment=QtCore.Qt.AlignVCenter)
                top_control.addStretch()
                layout.addLayout(top_control)

                main_rt_layout = QtWidgets.QHBoxLayout()
                main_rt_layout.setSpacing(20)

                # ⬅️ 左侧：通道库管理器
                channel_panel = QtWidgets.QWidget()
                channel_panel.setFixedWidth(350)
                channel_panel.setStyleSheet(
                    "background-color: #1E1F20; border: 1px solid #333537; border-radius: 12px;")
                channel_layout = QtWidgets.QVBoxLayout(channel_panel)
                channel_layout.setContentsMargins(20, 20, 20, 20)

                lbl_ch_title = QtWidgets.QLabel("📡 视频流矩阵控制")
                lbl_ch_title.setStyleSheet("color: #FFFFFF; font-size: 18px; font-weight: bold; border: none;")
                channel_layout.addWidget(lbl_ch_title)

                self.list_channels = QtWidgets.QListWidget()
                self.list_channels.setStyleSheet(
                    "background-color: #131314; color: #E3E3E3; border-radius: 6px; padding: 5px; font-size: 14px;")
                channel_layout.addWidget(self.list_channels, stretch=1)

                self.input_rtsp = QtWidgets.QLineEdit()
                self.input_rtsp.setPlaceholderText("输入网络 RTSP/HTTP 地址或本地视频")
                self.input_rtsp.setStyleSheet(
                    "QLineEdit { background-color: #131314; border: 1px solid #333537; border-radius: 6px; padding: 8px; font-size: 14px; color: #E3E3E3; }")
                channel_layout.addWidget(self.input_rtsp)

                btn_box1 = QtWidgets.QHBoxLayout()
                self.btn_browse_rt = QtWidgets.QPushButton("📂 浏览本地")
                self.btn_browse_rt.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
                self.btn_browse_rt.setStyleSheet(
                    "QPushButton { background-color: #2D2F31; color: white; border-radius: 6px; padding: 8px; font-weight:bold; } QPushButton:hover { background-color: #444746; }")

                self.btn_add_rt = QtWidgets.QPushButton("➕ 添加到通道库")
                self.btn_add_rt.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
                self.btn_add_rt.setStyleSheet(
                    "QPushButton { background-color: #004A77; color: white; border-radius: 6px; padding: 8px; font-weight:bold; } QPushButton:hover { background-color: #005B94; }")
                btn_box1.addWidget(self.btn_browse_rt)
                btn_box1.addWidget(self.btn_add_rt)
                channel_layout.addLayout(btn_box1)

                self.btn_remove_rt = QtWidgets.QPushButton("🗑️ 移除选中的通道")
                self.btn_remove_rt.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
                self.btn_remove_rt.setStyleSheet(
                    "QPushButton { background-color: transparent; border: 1px solid #7F1D1D; color: #EF4444; border-radius: 6px; padding: 8px; font-weight:bold; } QPushButton:hover { background-color: #7F1D1D; color: white; }")
                channel_layout.addWidget(self.btn_remove_rt)

                channel_layout.addSpacing(15)
                line = QtWidgets.QFrame()
                line.setFrameShape(QtWidgets.QFrame.HLine)
                line.setStyleSheet("background-color: #333537;")
                channel_layout.addWidget(line)
                channel_layout.addSpacing(15)

                self.btn_start_cams = QtWidgets.QPushButton("▶️ 一键启动监控矩阵")
                self.btn_start_cams.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
                self.btn_start_cams.setStyleSheet(
                    "QPushButton { background-color: #10B981; color: white; border-radius: 6px; padding: 12px; font-weight:bold; font-size: 16px; } QPushButton:hover { background-color: #059669; }")

                self.btn_stop_cams = QtWidgets.QPushButton("⏹️ 紧急中止所有通道")
                self.btn_stop_cams.setCursor(QtGui.QCursor(QtCore.Qt.PointingHandCursor))
                self.btn_stop_cams.setStyleSheet(
                    "QPushButton { background-color: #DC2626; color: white; border-radius: 6px; padding: 12px; font-weight:bold; font-size: 16px; } QPushButton:hover { background-color: #B91C1C; }")
                channel_layout.addWidget(self.btn_start_cams)
                channel_layout.addWidget(self.btn_stop_cams)
                main_rt_layout.addWidget(channel_panel)
                ...
                # 动态滚动区域 (存放无限网格)
                self.scroll_matrix = QtWidgets.QScrollArea()
                self.scroll_matrix.setWidgetResizable(True)
                self.scroll_matrix.setStyleSheet("QScrollArea { border: none; background-color: transparent; }")
                self.matrix_container = QtWidgets.QWidget()
                self.matrix_container.setStyleSheet("background-color: transparent;")
                self.matrix_layout = QtWidgets.QGridLayout(self.matrix_container)
                self.matrix_layout.setSpacing(15)
                self.matrix_layout.setContentsMargins(0, 0, 0, 0)
                self.scroll_matrix.setWidget(self.matrix_container)
                right_layout.addWidget(self.scroll_matrix, stretch=7)

                self.table_rt_results = QtWidgets.QTableWidget()
                self.table_rt_results.setObjectName("table_rt_results")
                self.table_rt_results.setColumnCount(5)
                self.table_rt_results.setHorizontalHeaderLabels(
                    ["报警时间", "监控源 (通道)", "事件类型", "置信度", "操作"])
                ...
                right_layout.addWidget(self.table_rt_results, stretch=3)

                main_rt_layout.addWidget(right_panel, stretch=1)
                layout.addLayout(main_rt_layout)
            ...
            # 👤 个人中心 与 ⚙️ 系统设置 与 🤖 AI助手 逻辑不变...
            # 为节省字符，下面加载个人中心/设置/AI的代码我保留你原本的写法：
            elif name in ["个人中心", "系统设置", "AI 助手"]:
                if name == "个人中心":
                    ...
                    self.btn_browse_path.setStyleSheet(
                        "QPushButton { background-color: #333537; color: white; border-radius: 4px; padding: 8px 15px; font-weight:bold; }")
                    ...
                    lay_alert.addWidget(
                        QtWidgets.QLabel("同目标重复报警冷却时间 (防止弹窗刷屏):", styleSheet=label_style))
                    ...
                    lay_ui.addWidget(QtWidgets.QLabel("客户端主题风格:", styleSheet=label_style))
